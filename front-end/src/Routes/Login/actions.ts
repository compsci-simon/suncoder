...
export const login = (username: string, password: string, id: string | null, afterEffect: (data: boolean) => void) => async (dispatch: AppDispatch) => {
  try {
    dispatch({ type: SHOW_BACKDROP })
    const { data } = await fastapi.user.login(username, password, id)
    afterEffect(data)
