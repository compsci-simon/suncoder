...
import app.util.feedback as properties
import re
...
class CFG:
    ...
    def _traverse(self, block: BasicBlock, visited: Set[int] = set(), calls: bool = True) -> None:
        if block.bid not in visited:
            ...
            if calls and block.calls:
                self.graph.node(str(
                    block.bid) + '_call', label=block.calls_to_code(), _attributes={'shape': 'box'})
                self.graph.edge(str(block.bid), str(
                    block.bid) + '_call', label="calls", _attributes={'style': 'dashed'})

            for next_bid in block.next:
                self._traverse(self.blocks[next_bid], visited, calls=calls)
                self.graph.edge(str(block.bid), str(next_bid), label=astor.to_source(
                    self.edges[(block.bid, next_bid)]) if self.edges[(block.bid, next_bid)] else '')

    def _show(self, fmt: str = 'pdf', calls: bool = True) -> gv.dot.Digraph:
        self.graph = gv.Digraph(name='cluster_'+self.name,
                                format=fmt, graph_attr={'label': self.name})
    ...
    # Erin: recursively traverses the cfg making calls to visitCode() and visitFunction() to populate the program and function dictionaries
    ...
    # Erin: traverses cfg to populate program dictionary
    ...
    def visitCode(self, prog: properties.program, block: BasicBlock, visited: Set[int] = set(), calls: bool = True) -> None:
        if block.bid not in visited:
            ...
            for next_bid in block.next:
                ...
                edgeB = properties.blockType.getType(
                    self.blocks[next_bid].stmts_to_code())
                ...
                # prog.props["edges"].append([edgeA, block.bid, edgeB, next_bid])
                self.visitCode(
                    prog, self.blocks[next_bid], visited, calls=calls)

    # Erin: traverses cfg to populate function dictionary
        ...
        if block.bid not in visited:
            ...
            if bool(re.search("(\n|^)if(.)*:$", block.stmts_to_code())):
                ...
                if prog.props["recursion"]:
                    prog.props["base"] = True
            if block.calls and block.calls_to_code() != prog.name:
                prog.props["calls"].append(block.calls_to_code())
...
class CFGVisitor(ast.NodeVisitor):
    ...
    def add_subgraph(self, tree: Type[ast.AST]) -> None:
        self.cfg.func_calls[tree.name] = CFGVisitor().build(
            tree.name, ast.Module(body=tree.body))
            ...
            if block.is_empty():
                for prev_bid in block.prev:
                    ...
                    for next_bid in block.next:
                        ...
                        self.add_edge(prev_bid, next_bid, self.add_condition(self.cfg.edges.get(
                            (prev_bid, block.bid)), self.cfg.edges.get((block.bid, next_bid))))
                ...
                for next_bid in block.next:
                    self.remove_empty_blocks(
                        self.cfg.blocks[next_bid], visited)
                block.next.clear()
    ...
    def invert(self, node: Type[ast.AST]) -> Type[ast.AST]:
        if type(node) == ast.Compare:
            if len(node.ops) == 1:
                ...
                tmpNode = ast.BoolOp(op=ast.And(), values=[ast.Compare(
                    left=node.left, ops=[node.ops[0]], comparators=[node.comparators[0]])])
                for i in range(0, len(node.ops) - 1):
                    tmpNode.values.append(ast.Compare(left=node.comparators[i], ops=[
                                          node.ops[i+1]], comparators=[node.comparators[i+1]]))
    ...
    def visit_Assign(self, node):
        ...
        if type(node.value) in [ast.ListComp, ast.SetComp, ast.DictComp, ast.GeneratorExp, ast.Lambda] and len(node.targets) == 1 and type(node.targets[0]) == ast.Name:
            if type(node.value) == ast.ListComp:
                self.add_stmt(self.curr_block, ast.Assign(targets=[ast.Name(
                    id=node.targets[0].id, ctx=ast.Store())], value=ast.List(elts=[], ctx=ast.Load())))
                ...
                self.add_stmt(self.curr_block, ast.Assign(targets=[ast.Name(
                    id=node.targets[0].id, ctx=ast.Store())], value=ast.Dict(keys=[], values=[])))
                self.dictCompReg = (node.targets[0].id, node.value)
            elif type(node.value) == ast.GeneratorExp:
                self.add_stmt(self.curr_block, ast.Assign(targets=[ast.Name(id=node.targets[0].id, ctx=ast.Store())], value=ast.Call(
                    func=ast.Name(id='__' + node.targets[0].id + 'Generator__', ctx=ast.Load()), args=[], keywords=[])))
    ...
    def visit_Break(self, node):
        ...
        self.add_edge(self.curr_block.bid,
                      self.loop_stack[-1].bid, ast.Break())
    ...
    def visit_DictComp(self, node):
        try:  # try may change to checking if self.dictCompReg exists
            self.generic_visit(ast.Module(
                self.visit_DictComp_Rec(self.dictCompReg[1].generators)))
        except:
            pass
        finally:
    ...
    def visit_For(self, node):
        ...
        if not node.orelse:
            ...
            orelse_block = self.add_edge(
                self.curr_block.bid, self.new_block().bid, ast.Name(id='else', ctx=ast.Load()))
            self.loop_stack.append(afterfor_block)
            self.curr_block = for_block
    ...
    def visit_GeneratorExp(self, node):
        try:  # try may change to checking if self.genExpReg exists
            self.generic_visit(ast.FunctionDef(name='__' + self.genExpReg[0] + 'Generator__',
                                               ...
                                               body=self.visit_GeneratorExp_Rec(
                                                   self.genExpReg[1].generators),
    ...
    def visit_Lambda(self, node):  # deprecated since there is autopep8
        self.add_subgraph(ast.FunctionDef(name=self.lambdaReg[0], args=node.args, body=[
                          ast.Return(value=node.body)], decorator_list=[], returns=None))
        self.lambdaReg = None
    ...
    def visit_ListComp(self, node):
        try:  # try may change to checking if self.listCompReg exists
            self.generic_visit(ast.Module(
                self.visit_ListComp_Rec(self.listCompReg[1].generators)))
        except:
            pass
        finally:
    ...
    def visit_SetComp(self, node):
        try:  # try may change to checking if self.setCompReg exists
            self.generic_visit(ast.Module(
                self.visit_SetComp_Rec(self.setCompReg[1].generators)))
        except:
            pass
        finally:
    ...
    def visit_Try(self, node):
        loop_guard = self.add_loop_block()
        self.curr_block = loop_guard
        self.add_stmt(loop_guard, ast.Try(
            body=[], handlers=[], orelse=[], finalbody=[]))

        after_try_block = self.new_block()
        self.add_stmt(after_try_block, ast.Name(
            id='handle errors', ctx=ast.Load()))
        ...
        if node.handlers:
            for handler in node.handlers:
                ...
                self.add_stmt(after_handler_block, ast.Name(
                    id='end except', ctx=ast.Load()))
        ...
        if node.orelse:
            ...
            self.add_stmt(after_else_block, ast.Name(
                id='end no error', ctx=ast.Load()))
    ...
    def visit_While(self, node):
        ...
        if not node.orelse:
            ...
            if not (isinstance(inverted_test, ast.NameConstant) and inverted_test.value == False):
                self.add_edge(self.curr_block.bid,
                              afterwhile_block.bid, inverted_test)
            ...
            if not (isinstance(inverted_test, ast.NameConstant) and inverted_test.value == False):
                self.add_edge(self.curr_block.bid,
                              orelse_block.bid, inverted_test)
            self.curr_block = self.add_edge(
                self.curr_block.bid, self.new_block().bid, node.test)
